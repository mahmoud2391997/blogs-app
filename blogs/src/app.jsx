...
//  sessionStorage.getItem("auth") ? null : .map((user)=> {user.state = "logged out"})
  const [authorized,setAuthorized] = useState(users ? users.find((user)=>user.email == authUser) ? users.find((user)=>user.email == authUser).state == "logged in" : false :false);
  ...
  function confirmLogout() {
    let  auth = JSON.parse(localStorage.getItem("users")).find((user)=>user.email == sessionStorage.getItem("auth"))
    auth.state = "logged out";
    console.log(auth);
    let users = JSON.parse(localStorage.getItem("users")).filter((user)=>user.email != auth.email);
    users.push(auth)
  console.log(users);
...
useEffect(()=>{
  ...
  return (
    ...
    <BlogComponent  user={users.find((user)=>user.state == "logged in")}/>
    </>
  )
}
